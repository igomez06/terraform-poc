...
resource "ansible_host" "bastion" {
  ...
  variables = {
    # Connection vars.
    ansible_user =  var.aws_ssh_user # Depends on the OS (admin for Debian).
    ansible_host = aws_instance.server[0].public_ip
...
resource "aws_instance" "agent" {
  ...
  tags = {
    Name  = "tf-israel-agent-${count.index}"
  }
}
